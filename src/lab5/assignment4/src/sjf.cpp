...
void SJFProgramManager::initialize() {
	running = nullptr;
	initializePCB();
}

int SJFProgramManager::executeThread(ThreadFunction function, void* parameter, const char* name, int priority) {
	// 关中断，防止创建线程的过程被打断
	...
	thread->ticks = std::numeric_limits<int>::max(); // 非抢占式的SJF同样不需要时间片
