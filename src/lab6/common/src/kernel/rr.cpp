...
void RRProgramManager::initialize() {
	running = nullptr;
	initializePCB();
}

int RRProgramManager::executeThread(ThreadFunction function, void* parameter, const char* name, int priority) {
	// 关中断，防止创建线程的过程被打断
...
void RRProgramManager::schedule() {
	bool status = interruptManager.getInterruptStatus();
	interruptManager.disableInterrupt();
...
void RRProgramManager::MESA_WakeUp(PCB *program) {
