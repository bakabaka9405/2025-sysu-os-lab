...
uint8* const screen = (uint8*)0xc00b8000;

void vprintf(const char* format, va_list args);

void STDIO::initialize() {
	// 清空屏幕
	for (uint i = 0; i < 25 * 80; ++i) {
		screen[2 * i] = ' ';
		screen[2 * i + 1] = 0x07; // 默认颜色为白色
	}
	moveCursor(0);
	systemService.setSystemCall(1, (void*)vprintf);
}
...
void printf(const char* format, ...) {
	...
	asm_system_call(1, (int)format, (int)args);
}
