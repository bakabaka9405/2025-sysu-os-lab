...
void MemoryManager::initialize() {
	...
	interruptManager.setInterruptDescriptor(14, (uint32)asm_page_fault_handler, 0);
}
...
uint MemoryManager::allocatePhysicalPages(enum AddressPoolType type, const int count) {
	...
	for (int i = 0; i < count; i++) {
		setRefCount(start + i * PAGE_SIZE, 1);
	}
...
void MemoryManager::releasePhysicalPages(enum AddressPoolType type, uint startAddress, const int count) {
	printf("release physical pages: start address=0x%x, count=%d\n", startAddress, count);
	if (type == AddressPoolType::KERNEL) {
...
extern "C" void c_page_fault_handler(uint errorCode, uint addr) {
	...
	memoryManager.connectPhysicalVirtualPage(addr, paddr_new);			   // 将原虚拟页重新映射到新物理页
	memoryManager.releaseVirtualPages(AddressPoolType::USER, addr_new, 1); // 释放中转虚拟页
	programManager.activateProgramPage(programManager.running);			   // 激活页表刷新TLB
