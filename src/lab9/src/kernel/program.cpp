...
void load_process(const char* filename) {
	...
	printf("esp: vaddr=0x%x, paddr=0x%x\n", interruptStack->esp, memoryManager.vaddr2paddr(interruptStack->esp - PAGE_SIZE));
	...
	if (program->pageDirectoryAddress) {
		...
		for (int i = 0; i < 768; ++i) {
			...
			for (int j = 0; j < 1024; ++j) {
				...
				if (memoryManager.decRefCount(paddr) == 0) {
